...
# 1. Filtrar apenas os pares que tiveram match na 'final_df'
matched_pairs_only_df = final_df[final_df["predicted_match"] == 1].copy()

# 2. Unir os detalhes do hotels_A para os hotéis que foram matchados
hotels_A_for_enrichment = matched_pairs_only_df.merge(
    hotels_A_details,
    on='id_A',
    how='left'
)
...
if API_URL_AVAILABLE:
    for index, row in unique_profiles_to_enrich.iterrows():
        payload = {
...
# ================================================================
df_final_consolidated = final_df[['id_A', 'id_B', 'predicted_match']].copy()

# 1. Adicionar as colunas descritivas
columns_for_consolidation = [
    'id_A', 'hotel_name', 'address', 'city', 'country',
    'latitude', 'longitude', 'rating', 'reviews_count'
]
df_final_consolidated = df_final_consolidated.merge(
    hotels_A[columns_for_consolidation],
    on='id_A',
    how='left'
)

# 2. Adicionar os dados enriquecidos da API.
df_final_consolidated = df_final_consolidated.merge(
    enriched_df_map,
    on=['hotel_name', 'city', 'country'],
    how='left'
)

# 3. Para os pares onde 'predicted_match' é 0, definir as colunas descritivas
columns_to_clear_for_no_match = [
    'hotel_name', 'address', 'city', 'country', 'latitude', 'longitude',
    'rating', 'reviews_count', 'enriched_category_stars',
    'enriched_review_score', 'enriched_amenities'
]

for col in columns_to_clear_for_no_match:
    df_final_consolidated.loc[df_final_consolidated['predicted_match'] == 0, col] = pd.NA

# 4. Reordenar as colunas para melhor visualização
final_columns_order = [
    'id_A', 'id_B', 'predicted_match',
    'hotel_name', 'address', 'city', 'country',
    'latitude', 'longitude', 'rating', 'reviews_count',
    'enriched_category_stars', 'enriched_review_score', 'enriched_amenities'
]
df_final_consolidated = df_final_consolidated[final_columns_order]

# Exportar apenas os pares matchados (predicted_match = 1)
